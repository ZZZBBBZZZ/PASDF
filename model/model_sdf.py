...
class SDFModel_ofo(torch.nn.Module):
    def __init__(self, num_layers, skip_connections, latent_size, inner_dim=512, output_dim=1):
        """
        SDF model for multiple shapes.
        Args:
            input_dim: 128 for latent space + 3 points = 131
        """
        super(SDFModel_ofo, self).__init__()
        ...
        self.latent_size = 0   # 去除latent_code
...
class SDFModel_PoseEmbedder(torch.nn.Module):
    def __init__(self, num_layers, skip_connections, PoseEmbedder_size,latent_size, inner_dim=512, output_dim=1):
        """
        SDF model for multiple shapes.
        Args:
            input_dim: 60 for PoseEmbedder + 3 points = 131
        """
        super(SDFModel_PoseEmbedder, self).__init__()
        ...
        # Dimension of the input space (3D coordinates)
        dim_coords = 3 
        input_dim = self.PoseEmbedder_size + dim_coords + self.latent_size
        print(input_dim)
