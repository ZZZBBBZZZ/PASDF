...
dataset_path = os.path.join(current_dir, "ShapeNetAD")
...
def main(cfg):
    obj_paths = sorted(glob(os.path.join(dataset_path, '*', '*.obj')))
    samples_dict = {}
    idx_str2int_dict = {}
    idx_int2str_dict = {}
    ...
    for obj_idx, obj_path in enumerate(obj_paths):
        ...
        try:
            ...
            # print(mesh_original.is_volume, mesh_original.is_watertight)

            if not mesh_original.is_watertight:
                verts, faces = pcu.make_mesh_watertight(
    ...
    os.makedirs(os.path.join(results_path, "ShapeNetAD"), exist_ok=True)

    np.save(os.path.join(results_path, "ShapeNetAD", f'samples_dict_{cfg["dataset"]}.npy'), samples_dict)
    np.save(os.path.join(results_path, "ShapeNetAD", 'idx_str2int_dict.npy'), idx_str2int_dict)
    np.save(os.path.join(results_path, "ShapeNetAD", 'idx_int2str_dict.npy'), idx_int2str_dict)

if __name__ == '__main__':
    cfg_path = os.path.join(config_files_path, 'extract_sdf_ShapeNetAD.yaml')
